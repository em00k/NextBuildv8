...
asm 
	...
	NextReg TURBO_CONTROL_NR_07,3								; 14mhz
	NextReg GLOBAL_TRANSPARENCY_NR_14,$0  						; Global transparency bits 7-0 = Transparency color value (0xE3 after a reset)
	NextReg PALETTE_INDEX_NR_40,0  							; 16 = paper 0 on L2 palette index 
	NextReg PALETTE_VALUE_NR_41,$0  						;	Palette Value 
	NextReg SPRITE_CONTROL_NR_15,%00001011
	NextReg LAYER2_CONTROL_NR_70,%00_01_0000
	NextReg TRANSPARENCY_FALLBACK_COL_NR_4A,0							; Trasnparent Fallback
	NextReg LAYER2_RAM_BANK_NR_12,10							; layer2 rams   16kb banks 
	NextReg LAYER2_RAM_SHADOW_BANK_NR_13,12  						; layer2 shadow
	NextReg PALETTE_CONTROL_NR_43,%00010000					; layer2 shadow
...
'    InitSprites(2,@Sprites)
    ...
    ' UpdateSprite(64,64,0,0,0,0) 

	' set up a test loop for smoothness 

    ' for x = 0 to 10*16 step 16
    '     UpdateSprite(64,y,1+x,0,0,relative) 
    ' next x 
...
do 
	...
	elseif a = KEYW and key = 0 
        NextReg($12,15)
	'	key 	= 1 
	elseif a = KEYE and key = 0 
        NextReg($12,20)
	'	key 	= 1 
	elseif a = KEYR and key = 0 
        NextReg($12,25)
	'	key 	= 1 
	elseif a = KEYT and key = 0 
        NextReg($12,30)
	'	key 	= 1 
	elseif a = KEYSPACE and key = 0 
        timer = 0 
	'	key 	= 1 
...
sub fastcall genpalette(offset as ubyte)
    asm 
        ;  BREAK 
        nextreg $52,19 
        ld      hl, $4000
...
SUB PalCycle(ByVal address as uinteger, byval colours as ubyte,byval offset as ubyte)
	...
	asm 
		...
		; a = offset  
		; de = user selected palette range
		; hl = buffer of range twice 
		; BREAK 
	loadpal:
		; push bc 
        push hl 
...
sub dostuff()
    'y = y + 1 
    'UpdateSprite(64,y,0,0,0,0) 
    PalCycle(@rainbow,31,0)
end sub 
...
skipload:
	...
	end_player:
		...
		;ld      h,a 							; for display timer on screen 
		;add     a,80
		;and     $fe 
