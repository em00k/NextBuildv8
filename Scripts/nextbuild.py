...
def CopyToDestination():
    ...
    if copy == 1:
        ...
        try:
            ...
            if autostart == 1 : 
                    shutil.copy(head_tail[0]+'/loader.bas', filename_path[0]+'nextzxos/autoexec.bas')
...
def AnythingToRun():
        ...
        if master != '' and noemu == 0:
          #  print(master)
            cmd = BASE_DIR+'/Emu/Cspect/Cspect.exe -w3 -16bit -fill=0 -brk -tv -vsync -analytics -nextrom -map='+head_tail[0]+'/'+filenamenoext+'.bas.map -zxnext -mmc='+head_tail[0]+'/data/ '+head_tail[0]+'/'+master
